...
export const course: string = (window as any).ENV.context_asset_string;
export const courseId = course.split("_")[1];
export const baseUrl: string = (window as any).ENV.DEEP_LINKING_POST_MESSAGE_ORIGIN;
export const baseApiUrl = baseUrl + "/api/v1/courses/";
...
export function getAll(verb: HandleResult, url: string, options: object): JQueryPromise<any> {
    return getAllWithoutCourse(verb, baseApiUrl + courseId + "/" + url, options);
}
